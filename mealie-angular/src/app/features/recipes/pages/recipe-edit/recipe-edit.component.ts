...
export class RecipeEditComponent implements OnInit, OnDestroy {
    ...
    private populateForm(recipe: Recipe): void {
        ...
        // Set selected categories, tags, tools, foods
        this.selectedCategories = recipe.categories.map(c => c.id);
        this.selectedTags = recipe.tags.map(t => t.id);
        this.selectedTools = recipe.tools.map(t => t.id);
        this.selectedFoods = recipe.foods.map(f => f.id);
    ...
    addIngredient(ingredient?: RecipeIngredient): void {
        const ingredientGroup = this.fb.group({
            ...
            quantity: [ingredient?.quantity || 1, [Validators.required, Validators.min(0)]],
            disableAmount: [ingredient?.disableAmount || false]
        });
    ...
    addInstruction(instruction?: RecipeInstruction): void {
        const instructionGroup = this.fb.group({
            text: [instruction?.text || '', [Validators.required]],
            position: [instruction?.position || this.instructions.length]
        });
